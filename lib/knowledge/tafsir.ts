...
export type TafsirAuthor = "ibn-kathir" | "saddi";
export type TafsirLang = "ar" | "en" | "ru";

const FOLDER: Record<`${TafsirAuthor}:${TafsirLang}`, string | null> = {
...
const NATIVE_PREFERENCE: Record<string, TafsirLang[]> = {
  ...
  en: ["en", "ar"],
  ar: ["ar", "en"],
  // Locales without a dedicated translation get the Arabic version of
  // each tafsir with a UI note. For Ibn Kathir we prefer EN first
  // because most non-Arab Muslim readers find it more useful.
  fa: ["ar", "en"],
  tg: ["ru", "ar"],
  uz: ["ru", "ar"],
  kk: ["ru", "ar"],
...
function pickLang(author: TafsirAuthor, locale: string): TafsirLang | null {
  ...
  for (const lang of order) {
    if (FOLDER[`${author}:${lang}`]) return lang;
  }
  return null;
}

interface TafsirFile {
  ayahs?: Array<{ ayah: number; surah: number; text: string }>;
}
...
export async function getTafsir(
  ...
  if (!lang) return null;
  const folder = FOLDER[`${author}:${lang}`];
